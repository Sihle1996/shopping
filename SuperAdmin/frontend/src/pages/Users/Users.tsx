...
import { Search, Trash2, AlertTriangle, UserCog } from 'lucide-react'
...
export default function Users() {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Role',
      render: (row: UserDto) => (
        <Badge label={row.role} variant={roleVariant(row.role)} />
      )
    },
    {
      key: 'tenantName',
    ...
    },
    {
      ...
      render: (row: UserDto) => (
        <div className="flex items-center gap-1">
          <button
            onClick={() => { setRoleUser(row); setNewRole(row.role) }}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="text-sm text-gray-500">{data?.total ?? 0} users</span>
      </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Updating role for <strong>{roleUser?.email}</strong>
          </p>
          <div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button
              onClick={() => roleUser && updateMutation.mutate({ id: roleUser.id, role: newRole })}
